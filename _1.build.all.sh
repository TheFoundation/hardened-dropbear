...
(
    ...
    docker export $(docker create --name cicache_${IMAGETAG//[:\/]/_}_${TARGETARCH} ${IMAGETAG}_${TARGETARCH} /bin/false ) |tar xv ccache.tgz ;docker rm cicache_${IMAGETAG//[:\/]/_}_${TARGETARCH}
     test -e ccache.tgz ||    (  (echo FROM ${IMAGETAG}_${TARGETARCH};echo RUN echo yocacheme) | time docker buildx build  --output=type=local,dest=/tmp/buildout_${IMAGETAG}_${TARGETARCH}  --progress plain --network=host --memory-swap -1 --memory 1024 --platform=${BUILDARCH}   --cache-from ${IMAGETAG}_${TARGETARCH}_buildcache -t  ${IMAGETAG}_${TARGETARCH} $buildstring -f - ) ;
...
##### multi arch
     test -e binaries.tgz ||    (  time docker buildx build  --output=type=local,dest=/tmp/buildout_${IMAGETAG}_${TARGETARCH}   --progress plain --network=host --memory-swap -1 --memory 1024 --platform=${BUILDARCH}   --cache-from ${IMAGETAG}_${TARGETARCH}_buildcache -t  ${IMAGETAG}_${TARGETARCH} $buildstring -f "${DFILENAME}" ) ;
     test -e /tmp/buildout_${IMAGETAG}_${TARGETARCH} && test -e /tmp/buildout_${IMAGETAG}_${TARGETARCH}/binaries.tgz && mv /tmp/buildout_${IMAGETAG}_${TARGETARCH}/binaries.tgz .
     test -e /tmp/buildout_${IMAGETAG}_${TARGETARCH} && rm -rf "/tmp/buildout_${IMAGETAG}_${TARGETARCH}"
